...
@section('title', 'Editar Borrador de Venta — Lubricantes «El Cisne»')

@section('content')
<div class="container-fluid px-4 py-3" id="posContainer">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h1 class="h3 mb-0 fw-bold text-dark">✏️ Editar Borrador de Venta</h1>
            <p class="text-muted small mb-0">Borrador ID #{{ $sale->id }} — Modifique líneas o confirme la venta</p>
        </div>
        <div class="d-flex gap-2">
            <form method="POST" action="{{ route('sales.destroy', $sale) }}" onsubmit="return confirm('¿Está seguro de descartar este borrador?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-danger btn-sm">🗑️ Descartar Borrador</button>
            </form>
            <a href="{{ route('sales.index') }}" class="btn btn-outline-secondary btn-sm">📋 Historial</a>
        </div>
    </div>
    ...
    <form id="posForm" method="POST" action="{{ route('sales.update', $sale) }}">
        @csrf
        @method('PUT')
        <div class="row g-3">
            {{-- Panel Izquierdo --}}
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm mb-3">
                    <div class="card-body">
                        ...
                        <div class="input-group input-group-lg position-relative mb-2">
                            ...
                            <input type="text" id="productSearch" class="form-control" placeholder="Escriba o escanee..." autocomplete="off">
                        </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white py-3 border-0">
                        <h5 class="card-title mb-0 fw-bold">📦 Detalle del Borrador</h5>
                    </div>
                ...
                <div class="card border-0 shadow-sm sticky-top" style="top: 80px;">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between fs-5 mb-2">
                            ...
                            <span id="posSubtotal" class="fw-bold">${{ number_format((float)$sale->subtotal, 2) }}</span>
                        </div>
                        <hr>
                        <div class="d-flex justify-content-between fs-3 fw-bold text-success mb-4">
                            <span>TOTAL:</span>
                            <span id="posTotal">${{ number_format((float)$sale->total, 2) }}</span>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <button type="button" id="btnConfirmSale" class="btn btn-success btn-lg fw-bold" data-bs-toggle="modal" data-bs-target="#confirmModal">
                                ✅ Confirmar Venta
                            </button>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body py-4">
                <p class="fs-5 text-center mb-2">¿Está seguro de confirmar esta venta por <strong id="modalTotalAmount">${{ number_format((float)$sale->total, 2) }}</strong>?</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load initial details from server PHP
    let cart = [
        @foreach ($sale->details as $d)
        {
            id: {{ $d->product_id }},
            name: @json($d->product_name),
            sku: @json($d->product_sku),
            unit: @json($d->unit),
            price: "{{ number_format((float)$d->unit_price, 2, '.', '') }}",
            quantity: "{{ number_format((float)$d->quantity, 3, '.', '') }}"
        },
        @endforeach
    ];
    ...
    btnExecuteConfirm.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                return fetch(`/sales/{{ $sale->id }}/confirm`, {
                    method: 'POST',
                    headers: {
                ...
                throw new Error(data.message || 'Error al actualizar borrador');
            }
        })
        .then(res => res.json())
        .then(confirmData => {
