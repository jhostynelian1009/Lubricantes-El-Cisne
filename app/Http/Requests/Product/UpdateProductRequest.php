...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $skuRules = ['required', 'string', 'max:60', Rule::unique('products', 'sku')->ignore($productId)];

        if ($product instanceof Product && $product->hasMovements()) {
            $skuRules[] = function ($attribute, $value, $fail) use ($product) {
                if ($value !== $product->sku) {
                    $fail('El código SKU no se puede modificar porque el producto ya posee movimientos de inventario.');
                }
            };
        }
